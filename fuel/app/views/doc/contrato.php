...
/* manifiestan */
$pdf->SetFont('Arial','B',12);
$pdf->Cell(0,5,'MANIFIESTAN',0,1,'C');
$pdf->Ln(3);

$pdf->SetFont('Arial','B',12);
$pdf->Cell(0,5,'PRIMERA',0,1,'L');
$pdf->SetFont('Arial','',12);
...
if(strpos($nombre,"LOPD")>=0 && strpos($nombre,"LOPD")!==false) {
    ...
    $pdf->Ln(5);
    $pdf->MultiCell(0, 6, utf8_decode('- Recordatorio mensual mediante correo electrónico de los controles periódicos y tareas de revisión que deberá realizar el BENEFICIARIO para detectar posibles cambios.'), 0);
    $pdf->Ln(5);
...
$pdf->MultiCell(0, 6, utf8_decode('El precio fijado para los servicios descritos, a ser percibido por ANÁLISIS Y GESTIÓN DE DATOS S.L., asciende a un total de '.$servicios_data[6]["precio"].' EUROS, impuestos no incluidos, que serán facturados anualmente por el PRESTATARIO.'), 0);
$pdf->Ln(5);
$pdf->MultiCell(0, 6, utf8_decode('La primera domiciliación será girada por el PRESTATARIO durante los cinco primeros días del mes de '.$meses[$servicios_data[6]["mes_factura"]-1].' de '.$servicios_data[6]["year"].', en la cuenta del BENEFICIARIO con código IBAN nº '.$customer->iban.'.'),0);
$pdf->Ln(5);
