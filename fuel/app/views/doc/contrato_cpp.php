...
$pdf->Ln(5);
$coinc="";
if(strcmp($rep->nombre,$aaff_nombre)!=0){
    $coinc= ', como representante legal de '.html_entity_decode($aaff_nombre);
}
$pdf->SetFont('Arial','',12);
$pdf->MultiCell(0,6,utf8_decode('          De una parte, '.$tratamiento_ops[$rep->tratamiento] .' '. html_entity_decode($rep->nombre).', mayor de edad, con DNI nº '.$rep->dni.$coinc.', y mandatario de la Comunidad de Propietarios '.html_entity_decode($customer->nombre).', con domicilio en '.html_entity_decode($customer->direccion).', C.P '.$customer->cpostal.', en '.html_entity_decode($customer->loc).', provincia de '.html_entity_decode($customer->prov).' y con CIF nº '.$customer->cif_nif.' (en adelante, EL BENEFICIARIO).'),0);
$pdf->Ln(5);
