...
<?php if ($facturas): ?>
	<table class="table table-striped">
		...
		<?php foreach ($facturas as $item): ?>
			<tr>
				<td><?php
					if($serv=Model_Servicios_Contratado::find($item->idsc)==NULL){
						echo '<span class="red">ERROR#1</span>';
					}
					else {
						if($c=Model_Cliente::find(Model_Contrato::find(Model_Servicios_Contratado::find($item->idsc)->get('idcontrato'))->get('idcliente'))) {
							echo Model_Cliente::find(Model_Contrato::find(Model_Servicios_Contratado::find($item->idsc)->get('idcontrato'))->get('idcliente'))->get('nombre');
						}
						else{
							echo '<span class="red">ERROR#2</span>';
						}
					}?></td>
				<td><?php
