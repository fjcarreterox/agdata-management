...
class PDFp extends PDF_MC_Table
{
    ...
    function Header(){
        ...
        //$this->Cell(0,25,date("d-m-Y",time()),0,0,'R');
        $this->Ln(5);
        $this->SetFont('Arial','I',13);
        $this->Cell(0,45,utf8_decode("                         ".html_entity_decode($nombre)),0,0,'C');
        $this->Ln(10);
        $this->Ln(10);
...
$repseg=json_decode(urldecode($repseg_data),true);
$ficheros=json_decode($f_data,true);
$ces = json_decode(html_entity_decode(urldecode($cesionarias)),true);
$nombre = html_entity_decode(urldecode($nombre));
...
$pdf->MultiCell(0,12,mb_strtoupper(utf8_decode($nombre)),0,'C');
$pdf->Ln(15);

$fecha = date("m-Y",time());
$fecha_array=explode('-',$fecha);
$mes = $pdf->getMes($fecha_array[0]);
...
$pdf->MultiCell(0,6,utf8_decode('En este Documento de Seguridad se recogerán todas aquellas medidas de índole técnica y organizativa que deben reunir los ficheros, locales, equipos informáticos y el personal de '.$nombre.' situado en '.$cliente["dir"].' con C.P. '.$cliente["cp"].' en '.$cliente["loc"].', provincia de '.$cliente["prov"].'.'),0,'J');$pdf->Ln(2.5);
...
if(count($ficheros>1)){$str=count($ficheros)." ficheros con datos de carácter personal, los cuales han sido convenientemente notificados";}
$pdf->MultiCell(0,6,utf8_decode($cliente["nombre"].' mantiene '.$str.' a la Agencia Española de Protección de Datos para su inscripción en el Registro General de Protección de Datos.'),0,'J');$pdf->Ln(2.5);
...
foreach($ficheros as $f){
    ...
    $pdf->SetWidths(array(60,55,55));
    $pdf->SetAligns(array('C','C','C'));
    $pdf->Row(array($f["nombre"],$f["soporte"],$f["nombre_nivel"]));

    $pdf->Ln(5);
}

$str="del fichero declarado";
if(count($ficheros>1)){$str="de los ficheros declarados";}
$pdf->MultiCell(0,6,utf8_decode('La descripción y tipología '.$str.' por '.$cliente["nombre"].' viene especificada en el Anexo I: "Descripción de los ficheros" del presente documento.'),0,'J');$pdf->Ln(2.5);
$niveles = array("N/D","BÁSICO","MEDIO","ALTO");
$pdf->MultiCell(0,6,utf8_decode('Una vez analizada la tipología de los ficheros a proteger, se recogerán en el Documento de Seguridad, con carácter general, los procedimientos y normas de seguridad establecidas como de nivel '.$niveles[$max_nivel].'.'),0,'J');$pdf->Ln(2.5);

$str1="del fichero identificado";
$str2="del fichero";
if(count($ficheros>1)){$str1="de los ficheros identificados";$str2="de los ficheros";}
...
$pdf->SetFont('Arial','',10);
$pdf->MultiCell(0,6,utf8_decode($nombre.', ha designado como Responsable de Seguridad a '.$repseg[0]["nombre"].', quien desempeñará las funciones propias de coordinación y control de las medidas de seguridad implantadas en materia de protección de datos.'),0,'J');$pdf->Ln(2.5);
...
$pdf->MultiCell(0, 6, utf8_decode('La administración y gestión de los ficheros de '.$cliente["nombre"].' está localizada en ' . html_entity_decode($cliente["dir"]) . ' con C.P. ' . $cliente["cp"] . ' en ' . $cliente["loc"] . ', provincia de ' . $cliente["prov"] . ', donde se encuentran ubicadas sus oficinas e instalaciones.'), 0, 'J');
$pdf->Ln(2.5);
...
//6.1
$pdf->SetFont('Arial','B',12);
$pdf->MultiCell(0,10,mb_strtoupper(utf8_decode('6. Gestión de incidencias')),0,'L');
...
//7.1
$pdf->SetFont('Arial','B',12);
$pdf->MultiCell(0,10,mb_strtoupper(utf8_decode('7. Gestión de soportes')),0,'L');
...
$pdf->MultiCell(0,6,utf8_decode('El Responsable de Seguridad de '.$nombre.' comprobará, con una periodicidad al menos trimestral, que la lista de personal con acceso autorizado del Anexo III se corresponde con la lista de los usuarios realmente autorizados en la aplicación de acceso a los ficheros.'),0,'J');$pdf->Ln(2.5);
...
$pdf->SetFont('Arial', 'B', 12);
$pdf->MultiCell(0, 6, mb_strtoupper(utf8_decode('     A/A:  ' . $cliente["nombre"])), 0, 'L');
$pdf->SetFont('Arial', '', 11);
$pdf->MultiCell(0, 6, mb_strtoupper(utf8_decode('              ' . $cliente["dir"])), 0, 'L');
$pdf->MultiCell(0, 6, mb_strtoupper(utf8_decode('              C.P.' . $cliente["cp"] . ', ' . $cliente["loc"] . ', ' . $cliente["prov"])), 0, 'L');
$pdf->Ln(5);
...
foreach($ficheros as $f){
    ...
    if($f["idtipo"]==3){
        ...
        $pdf->SetFont('Arial','B',10);
        $pdf->Row(array("Fichero:","PERSONAL","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:",$nombre,"Soporte papel / digital: ".strtoupper($f["soporte"])));
        $pdf->Row(array("Responsable Seguridad:",$repseg[0]["nombre"],"Nivel de seguridad: ".mb_strtoupper(utf8_decode($f["nombre_nivel"]))));
        $pdf->SetWidths(array(165));
        $pdf->SetAligns(array('L'));
        $pdf->Row(array(""));
        $pdf->SetWidths(array(50,115));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","GESTIÓN DEL PERSONAL, NÓMINAS Y FORMACIÓN DEL PERSONAL"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","ENCUESTAS / ENTREVISTAS"));
        $pdf->Ln(5);
        //structure table
        $pdf->SetWidths(array(165));
        ...
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Nombre y apellidos","De carácter identificativo","Básico"));
        $pdf->Row(array("Dirección","De carácter identificativo","Básico"));
        $pdf->Row(array("Teléfono","De carácter identificativo","Básico"));
        $pdf->Row(array("D.N.I.","De carácter identificativo","Básico"));
        $pdf->Row(array("Nº Seguridad Social","De carácter identificativo","Básico"));
        $pdf->Row(array("Datos bancarios","Económicos - financieros","Básico"));
        $pdf->Row(array("Profesión","De detalles de empleo","Básico"));
        $pdf->Row(array("Puestos de trabajo","De detalles de empleo","Básico"));
        $pdf->Ln(20);
    }
    //if clientes
    elseif($f["idtipo"]==1){
    //file table
        ...
        $pdf->SetWidths(array(50,50,65));
        $pdf->SetAligns(array('L','L','L'));
        $pdf->Row(array("Fichero:","CLIENTES","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:",$nombre,"Soporte papel / digital: ".strtoupper($f["soporte"])));
        $pdf->Row(array("Responsable Seguridad:",$repseg[0]["nombre"],"Nivel de seguridad: ".$f["nombre_nivel"]));
        $pdf->SetWidths(array(165));
        $pdf->SetAligns(array('L'));
        $pdf->Row(array(""));
        $pdf->SetWidths(array(50,115));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","GESTIÓN ADMINISTRATIVA Y CONTABLE DE LOS CLIENTES"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","FORMULARIOS / CUPONES"));
        $pdf->Ln(5);
        //structure table
        $pdf->SetWidths(array(165));
        $pdf->SetAligns(array('C'));
        ...
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Nombre y apellidos","De carácter identificativo","Básico"));
        $pdf->Row(array("D.N.I / C.I.F","De carácter identificativo","Básico"));
        $pdf->Row(array("Datos bancarios","Económicos / financieros","Básico"));
        $pdf->Row(array("Dirección","De carácter identificativo","Básico"));
        $pdf->Row(array("Teléfono","De carácter identificativo","Básico"));
        $pdf->Row(array("Correo electrónico","De carácter identificativo","Básico"));
        $pdf->Ln(20);
    }
    //if curriculums
    if($f["idtipo"]==4){
        ...
        $pdf->SetAligns(array('L','L','L'));
        $pdf->SetFont('Arial','B',10);
        $pdf->Row(array("Fichero:","CURRÍCULUMS","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:",$nombre,"Soporte papel / digital: ".strtoupper($f["soporte"])));
        $pdf->Row(array("Responsable Seguridad:",$repseg[0]["nombre"],"Nivel de seguridad: ".$f["nombre_nivel"]));
        $pdf->SetWidths(array(165));
        ...
        $pdf->SetWidths(array(50,115));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","SELECCIÓN DE PERSONAL Y GESTIÓN DE RECURSOS HUMANOS"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","ENCUESTAS / ENTREVISTAS"));
        $pdf->Ln(5);
        //structure table
        ...
        $pdf->Row(array("Teléfono","De carácter identificativo","Básico"));
        $pdf->Row(array("D.N.I.","De carácter identificativo","Básico"));
        $pdf->Row(array("Imagen / Voz","De carácter identificativo","Básico"));
        $pdf->Row(array("Estado civil","Características personales","Básico"));
        $pdf->Row(array("Sexo","Características personales","Básico"));
        $pdf->Row(array("Edad","Características personales","Básico"));
        $pdf->Row(array("Nacionalidad","Características personales","Básico"));
        $pdf->Row(array("Fecha nacimiento","Características personales","Básico"));
        $pdf->Row(array("Formación, titulaciones","Académicos - Profesionales","Básico"));
        $pdf->Row(array("Experiencia profesional","Académicos - Profesionales","Básico"));
        $pdf->Row(array("Historial del trabajador","Académicos - Profesionales","Básico"));
        $pdf->Ln(20);
    }
    elseif($f["idtipo"]==2){
    //file table
        ...
        $pdf->SetWidths(array(50,50,65));
        $pdf->SetAligns(array('L','L','L'));
        $pdf->Row(array("Fichero:","PROVEEDORES","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:",$nombre,"Soporte papel / digital: ".strtoupper($f["soporte"])));
        $pdf->Row(array("Responsable Seguridad:",$repseg[0]["nombre"],"Nivel de seguridad: ".$f["nombre_nivel"]));
        $pdf->SetWidths(array(165));
        ...
        $pdf->SetWidths(array(50,115));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","GESTIÓN ADMINISTRATIVA Y CONTABLE DE LOS PROVEEDORES"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","ENCUESTAS / ENTREVISTAS"));
        $pdf->Ln(5);
        //structure table
    ...
    //file table
        ...
        $pdf->SetWidths(array(50,50,65));
        $pdf->SetAligns(array('L','L','L'));
        $pdf->Row(array("Fichero:","ALUMNOS","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:",$nombre,"Soporte papel / digital: ".strtoupper($f["soporte"])));
        $pdf->Row(array("Responsable Seguridad:",$repseg[0]["nombre"],"Nivel de seguridad: ".strtoupper($f["nombre_nivel"])));
        $pdf->SetWidths(array(165));
        $pdf->SetAligns(array('L'));
        $pdf->Row(array(""));
        $pdf->SetWidths(array(50,115));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","CONTROL Y GESTIÓN DEL ALUMNADO DEL CENTRO"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","FORMULARIOS / CUPONES"));
        $pdf->Ln(5);
        //structure table
        ...
        $pdf->Row(array("Nombre y apellidos","De carácter identificativo","Básico"));
        $pdf->Row(array("D.N.I / C.I.F","De carácter identificativo","Básico"));
        $pdf->Row(array("Nº de Seguridad Social","De carácter identificativo","Básico"));
        $pdf->Row(array("Dirección","De carácter identificativo","Básico"));
        $pdf->Row(array("Teléfono","De carácter identificativo","Básico"));
        $pdf->Row(array("Imagen","De carácter identificativo","Básico"));
        $pdf->Row(array("Estado civil","Características personales","Básico"));
        $pdf->Row(array("Sexo","Características personales","Básico"));
        $pdf->Row(array("Edad","Características personales","Básico"));
        $pdf->Row(array("Nacionalidad","Características personales","Básico"));
        $pdf->Row(array("Fecha nacimiento","Características personales","Básico"));
        $pdf->Row(array("Características físicas, titulaciones","Características personales","Básico"));
        $pdf->Row(array("Datos de familia","Características personales","Básico"));        
        $pdf->Row(array("Historial del estudiante","Datos académicos","Básico"));
        $pdf->Ln(20);
    }
    elseif($f["idtipo"]==13){
    //file table
        ...
        $pdf->SetWidths(array(50,50,65));
        $pdf->SetAligns(array('L','L','L'));
        $pdf->Row(array("Fichero:","PADRES ALUMNOS","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:",$nombre,"Soporte papel / digital: ".strtoupper($f["soporte"])));
        $pdf->Row(array("Responsable Seguridad:",$repseg[0]["nombre"],"Nivel de seguridad: ".strtoupper($f["nombre_nivel"])));
        $pdf->SetWidths(array(165));
        ...
        $pdf->SetWidths(array(50,115));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","GESTIÓN ESCOLAR"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","FORMULARIOS / CUPONES"));
        $pdf->Ln(5);
        //structure table
        ...
        $pdf->Row(array("Dirección","De carácter identificativo","Básico"));
        $pdf->Row(array("Teléfono","De carácter identificativo","Básico"));
        $pdf->Row(array("Firma","De carácter identificativo","Básico"));
        $pdf->Row(array("Estado civil","Características personales","Básico"));
        $pdf->Row(array("Sexo","Características personales","Básico"));
        $pdf->Row(array("Edad","Características personales","Básico"));
        $pdf->Row(array("Nacionalidad","Características personales","Básico"));
        $pdf->Row(array("Fecha nacimiento","Características personales","Básico"));
        $pdf->Row(array("Datos de familia","Características personales","Básico"));        
        $pdf->Row(array("Formación, titulaciones","Datos académicos y profesionales","Básico"));
        $pdf->Row(array("Ingresos, rentas","Datos de detalles de empleo","Básico"));
        $pdf->Row(array("Datos no económicos de nómina","Datos de detalles de empleo","Básico"));
        $pdf->Row(array("Datos bancarios","Económicos - financieros","Básico"));
        $pdf->Ln(20);
    }
    elseif($f["idtipo"]==12){
    //file table
        ...
        $pdf->SetWidths(array(50,50,65));
        $pdf->SetAligns(array('L','L','L'));
        $pdf->Row(array("Fichero:","PROFESORADO","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:",$nombre,"Soporte papel / digital: ".strtoupper($f["soporte"])));
        $pdf->Row(array("Responsable Seguridad:",$repseg[0]["nombre"],"Nivel de seguridad: ".strtoupper($f["nombre_nivel"])));
        $pdf->SetWidths(array(165));
        ...
        $pdf->SetWidths(array(50,115));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","GESTIÓN ADMINISTRATIVA DEL PROFESORADO DEL CENTRO"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","ENCUESTAS / ENTREVISTAS"));
        $pdf->Ln(5);
        //structure table
        ...
        $pdf->Row(array("Datos de familia","Características personales","Básico"));        
        $pdf->Row(array("Formación, titulaciones","Datos académicos y profesionales","Básico"));
        $pdf->Row(array("Puestos de trabajo","Datos de detalles de empleo","Básico"));
        $pdf->Row(array("Profesión","Datos de detalles de empleo","Básico"));
        $pdf->Row(array("Datos no económicos de nómina","Datos de detalles de empleo","Básico"));
        $pdf->Row(array("Datos bancarios","Económicos - financieros","Básico"));
        $pdf->Row(array("Datos económicos de nómina","Económicos - financieros","Básico"));
        $pdf->Row(array("Ingresos y rentas","Económicos - financieros","Básico"));
        $pdf->Ln(20);
    }
}
...
$falta="";
if(strcmp($repseg[0]["falta"],"00-00-0000")!=0){$falta=$repseg[0]["falta"];}
$fbaja="";
if(strcmp($repseg[0]["fbaja"],"00-00-0000")!=0){$fbaja=$repseg[0]["fbaja"];}
$pdf->Row(array($repseg[0]["nombre"],$repseg[0]["cargo"],$falta,$fbaja));
$pdf->Row(array("","","",""));
$pdf->Row(array("","","",""));
$pdf->Ln(10);
...
foreach($trab as $t){
    ...
    $pdf->Row(array($t["nombre"],$t["cargo"],$t["dni"],$falta,$fbaja));
}
$pdf->Row(array("","","","",""));
$pdf->Row(array("","","","",""));
$pdf->Ln(10);
...
$pdf->Ln(3);
$pdf->MultiCell(0,6,utf8_decode('A/A:        '.$tipo.' '.$nombre),0,'L');
$pdf->SetFont('Arial','',10);
$pdf->MultiCell(0,6,utf8_decode('                 '.html_entity_decode(urldecode($cliente["dir"]))),0,'L');
$pdf->MultiCell(0,6,utf8_decode('                 '.$cliente["cp"].', '.$cliente["loc"].', '.$cliente["prov"]),0,'L');
$pdf->Ln(10);
$pdf->SetFont('Arial','B',11);
...
$pdf->Output("DOC-SEGURIDAD-".$nombre.".pdf",'I');