...
$pres = json_decode(html_entity_decode(urldecode($pres_data)),true);
...
$pdf->MultiCell(0,12,strtoupper($tipo),0,'C');
...
$pdf->Cell(5,6,mb_strtoupper(utf8_decode('2. ámbito de aplicación')),0,1,'L');
...
if(count($ficheros>1)){$str="mantienen ".count($ficheros)." ficheros con datos de carácter personal, los cuales han sido convenientemente notificados";}
...
$pdf->SetFont('Arial','',10);
$pdf->MultiCell(0,6,utf8_decode('La Comunidad de Propietarios '.$nombre.', designa como Responsable de Seguridad al Presidente vigente de la Comunidad, quien desempeñará las funciones propias de coordinación y control de las medidas de seguridad implantadas en materia de protección de datos.'),0,'J');$pdf->Ln(2.5);
...
$pdf->MultiCell(0, 6, utf8_decode('La administración y gestión de los ficheros de la Comunidad de Propietarios se lleva a cabo tanto en ' . html_entity_decode($cliente["dir"]) . ' con C.P. ' . $cliente["cp"] . ' en ' . $cliente["loc"] . ', provincia de ' . $cliente["prov"] . ', como en las instalaciones de su Administrador de Fincas, ' . $reps[0]["nombre_aaff"] . ', situadas en ' . $reps[0]["dir"] . ' con C.P. ' . $reps[0]["cp"] . ' en ' . $reps[0]["loc"] . ', provincia de ' . $reps[0]["prov"] . '.'), 0, 'J');
$pdf->Ln(2.5);
...
if($num_reps > 1) {
    $i=1;
    while($i<$num_reps) {
        $pdf->MultiCell(0, 6, utf8_decode('También en las instalaciones de su otra Administradora de Fincas, ' . $reps[$i]["nombre_aaff"] . ', situadas en ' . $reps[$i]["dir"] . ' con C.P. ' . $reps[$i]["cp"] . ' en ' . $reps[$i]["loc"] . ', provincia de ' . $reps[$i]["prov"] . '.'), 0, 'J');
        $pdf->Ln(2.5);
        $i++;
    }
}
...
//5.1
$pdf->SetFont('Arial','B',12);
$pdf->MultiCell(0,10,mb_strtoupper(utf8_decode('5. Gestión de incidencias')),0,'L');
...
//6.1
$pdf->SetFont('Arial','B',12);
$pdf->MultiCell(0,10,mb_strtoupper(utf8_decode('6. Gestión de soportes')),0,'L');
...
$pdf->SetFont('Arial', 'B', 12);
$pdf->MultiCell(0, 6, mb_strtoupper(utf8_decode('     A/A:  Comunidad de Propietarios ' . $cliente["nombre"])), 0, 'L');
$pdf->SetFont('Arial', '', 11);
...
$pdf->AddPage();
$pdf->SetFont('Arial','B',12);
$pdf->MultiCell(0,10,strtoupper(utf8_decode('9. anexos')),0,'L');
$pdf->SetFont('Arial','',10);
...
foreach($ficheros as $f){
    //if comunidad de propietarios
    if($f["idtipo"]==5){
        //file table
        $pdf->SetFont('Arial','B',10);
        $pdf->SetWidths(array(60,60,60));
        $pdf->SetAligns(array('L','L','L'));
        $pdf->Row(array("Fichero:","COMUNIDAD DE PROPIETARIOS","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:","CDAD. PROP. ".$nombre,"Soporte papel / digital: ".$f["soporte"]));
        $pdf->Row(array("Responsable Seguridad:","PRESIDENTE DE LA COMUNIDAD","Nivel de seguridad: ".$f["nombre_nivel"]));
        $pdf->SetWidths(array(180));
        $pdf->SetAligns(array('L'));
        $pdf->Row(array(""));
        $pdf->SetWidths(array(60,120));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","GESTIÓN GENERAL DE LA COMUNIDAD"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","ENCUESTA / ENTREVISTAS"));
        $pdf->Ln(5);
        //structure table
        $pdf->SetWidths(array(180));
        ...
        $pdf->Row(array("Número de cuenta bancaria","Económicos / financieros","Básico"));
        ...
        $pdf->Row(array("Correos electrónicos","De carácter identificativo","Básico"));
        $pdf->Ln(15);
    }
    //if personal
    elseif($f["idtipo"]==3){
    ...
    elseif($f["idtipo"]==6){
        ...
        $pdf->SetAligns(array('L','L','L'));
        $pdf->SetFont('Arial','B',10);
        $pdf->Row(array("Fichero:","VIDEOVIGILANCIA","Nº Inscripción R.G.P.D:"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Responsable Fichero:","CDAD. PROP. ".$nombre,"Soporte papel / digital: ".$f["soporte"]));
        $pdf->Row(array("Responsable Seguridad:","PRESIDENTE DE LA COMUNIDAD","Nivel de seguridad: ".$f["nombre_nivel"]));
        $pdf->SetWidths(array(180));
        ...
        $pdf->SetWidths(array(60,120));
        $pdf->SetAligns(array('L','L'));
        $pdf->Row(array("Finalidad:","VIDEOVIGILANCIA DE LAS INSTALACIONES DE LA COMUNIDAD"));
        $pdf->Row(array("Procedencia de los datos:","EL PROPIO INTERESADO O SU REPRESENTANTE LEGAL"));
        $pdf->Row(array("Procedimiento de recogida:","FORMULARIOS / ENTREVISTAS"));
        $pdf->Ln(5);
        //structure table
        $pdf->SetWidths(array(180));
        $pdf->SetAligns(array('C'));
        ...
        $pdf->Row(array("CAMPOS DE DATOS","TIPO DE DATOS","NIVEL DE SEGURIDAD"));
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array("Imágenes","De carácter identificativo","Básico"));
        $pdf->Ln(15);
    }
}
...
if(count($pres)>0){
    $pdf->Row(array($pres["nombre"],"PRESIDENTE",$pres["falta"],$pres["fbaja"]));
}
else{
    $pdf->Row(array("","","",""));
}
...
if(count($trab)>0){
    foreach($trab as $t) {
        $pdf->Row(array($t["nombre"], $t["cargo"], $t["falta"], $t["fbaja"]));
    }
}
else{
    $pdf->Row(array("","","",""));
...
$i=0;
while($i<$num_reps){
    $pdf->Row(array($reps[$i]["nombre"],$reps[$i]["dni"],"",""));
    $i++;
}
$pdf->Row(array("","","",""));
$pdf->Row(array("","","",""));
...
foreach($trab as $t) {
    ...
    $pdf->Ln(2);
    $pdf->MultiCell(0, 6, mb_strtoupper(utf8_decode($t["nombre"])).utf8_decode(', mayor de edad, con DNI nº '.$t["dni"].', en virtud de la relación de carácter laboral que le vincula a la Comunidad de Propietarios ').mb_strtoupper(utf8_decode($cliente["nombre"])).utf8_decode(', se obliga a:'), 0, 'J');
    $pdf->Ln(2);
    $pdf->MultiCell(0, 6, utf8_decode('PRIMERO.- Guardar secreto profesional con respecto a los datos de carácter personal a los que tenga acceso por razón de su trabajo, así como guardarlos; obligaciones que se mantendrán aún después del cese de la relación laboral que le vincula a la Comunidad de Propietarios ').mb_strtoupper(utf8_decode($cliente["nombre"])).'.', 0, 'J');
    $pdf->Ln(2);
    ...
    $pdf->Ln(2);
    $pdf->MultiCell(0, 6, utf8_decode('TERCERO.- Seguir las instrucciones de la Comunidad de Propietarios ').mb_strtoupper(utf8_decode($cliente["nombre"])).utf8_decode(' en relación a las políticas de protección de datos descritas en el documento de seguridad.'), 0, 'J');
    $pdf->Ln(2);
    $pdf->MultiCell(0, 6, utf8_decode('CUARTO.- Trasladar al Responsable de Seguridad cualquier comunicación que llegue a la Comunidad de Propietarios ').mb_strtoupper(utf8_decode($cliente["nombre"])).utf8_decode(', relativa al ejercicio de los derechos de acceso, rectificación, cancelación y oposición por parte de los afectados respecto a sus datos de carácter personal.'), 0, 'J');
    $pdf->Ln(2);
    ...
    $pdf->Ln(2);
    $pdf->MultiCell(0, 6, utf8_decode('Asimismo, y en cumplimiento de lo dispuesto en el artículo 5 de la Ley Orgánica 15/1999, de 13 de diciembre de Protección de Datos de Carácter Personal (LOPD), la Comunidad de Propietarios ').mb_strtoupper(utf8_decode($cliente["nombre"])).utf8_decode(', con CIF '.$cliente["cif_nif"].', le informa que sus datos de carácter personal, actualmente en posesión de la Comunidad de Propietarios ').mb_strtoupper(utf8_decode($cliente["nombre"])).utf8_decode(', formarán parte de un fichero automatizado del que es titular y único responsable.'), 0, 'J');
    $pdf->Ln(2);
...
$pdf->Output("DOC-SEGURIDAD-C.PP.-".$nombre.".pdf",'I');